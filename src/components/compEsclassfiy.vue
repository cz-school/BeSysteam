  ...
  <div class="menuList">
    ...
    <el-row :span="24">
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>
          <a :href="`${this.$route.matched[1].path}`">商品分类</a>
        </el-breadcrumb-item>
      </el-breadcrumb>
    </el-row>
    <!-- 搜索框  添加框-->
...
export default {
  ...
  methods: {
    ...
    change_classify(id) {
      // 通过id 获取到数据
      this.$axios.get(`/esclassify/${id}`).then(res => {
    ...
    changetwo_classify() {
      this.$refs.changeruleref.validate(valid => {
        if (!valid) {
          ...
          this.$axios
            .put(`/esclassify/${this.changeruleForm.id}`, this.changeruleForm)
            .then(res => {
              if (res.data.state.code !== "200") {
                return this.$message.error(res.data.state.msg);
              } else {
    ...
    add_classify() {
      this.$refs.addruleref.validate(valid => {
        if (!valid) {
          ...
          this.$axios.post("/esclassify", this.addruleForm).then(res => {
            if (res.data.state.code !== "200") {
              return this.$message.error(res.data.state.msg);
            } else {
              this.$message.success(res.data.state.msg);
    ...
    delete_classify(id) {
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$axios.delete(`/esclassify/${id}`).then(res => {
            if (res.data.state.code !== "200") {
              return this.$message.error(res.data.state.msg);
            } else {
              this.$message({
    ...
    async getclassify_st() {
      let res = await this.$axios.get("/esclassify", {
